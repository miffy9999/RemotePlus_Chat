...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    throw new GuestApiError(toJapaneseUserMessage(reason instanceof Error ? reason.message : reason), 0);
  }
  const body = await response.json().catch(() => ({}));
  if (!response.ok) { const message = Array.isArray(body.message) ? body.message.join(", ") : body.message; throw new GuestApiError(toJapaneseUserMessage(message, response.status), response.status); }
  return body as T;
}
