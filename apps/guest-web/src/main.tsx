...
function GuestApp(): React.JSX.Element {
  ...
  const [language, setLanguage] = useState("ko"); const [agreed, setAgreed] = useState(false); const [starting, setStarting] = useState(false);
  ...
  if (screen === "loading") return <div className="center-card"><div className="spinner"/><h1>상담을 준비하고 있습니다</h1><p>객실 접근 정보를 안전하게 확인하는 중입니다.</p></div>;
  if (screen === "consent") return <div className="center-card"><div className="consent-card"><span className="eyebrow">HOTEL SUPPORT</span><h1>실시간 상담 시작</h1><p>상담은 최대 15분 동안 진행되며 텍스트 메시지만 지원합니다.</p><label>사용 언어<select value={language} onChange={(event) => setLanguage(event.target.value)}><option value="ko">한국어</option><option value="en">English</option><option value="ja">日本語</option><option value="zh">中文</option></select></label><label className="agreement"><input type="checkbox" checked={agreed} onChange={(event) => setAgreed(event.target.checked)} /><span>상담 내용이 서비스 제공과 안전한 운영을 위해 저장될 수 있음을 확인했습니다. 비밀번호와 결제정보는 입력하지 않겠습니다.</span></label><button onClick={() => void startConsultation()} disabled={!agreed || starting}>{starting ? "상담 준비 중…" : "상담 시작"}</button></div></div>;
  if (screen === "error" || !access) return <div className="center-card error-state"><h1>상담을 시작할 수 없습니다</h1><p>{error}</p><code>?accessKey=demo-room-access-1201</code></div>;
  const writable = access.session.status === "ACTIVE" && new Date(access.session.expiresAt).getTime() > now;
  return <main className="guest-shell"><header><div><strong>호텔 고객지원</strong><span>{access.session.room.hotel.name} · {access.session.room.roomNumber}호</span></div><span className={`online ${connection === "연결됨" ? "ok" : ""}`}>● {connection}</span></header><section className={`notice ${access.session.status.toLowerCase()}`}>{access.session.status === "WAITING" ? "상담원 연결을 기다리고 있습니다. 연결되면 메시지를 보낼 수 있습니다." : access.session.status === "ACTIVE" ? "상담원이 연결되었습니다. 개인정보나 결제 비밀번호는 전송하지 마세요." : "상담이 종료되었습니다. 더 이상 메시지를 보낼 수 없습니다."}</section>{error && <section className="error-box">{error}</section>}<section className="messages">{messages.length === 0 && <div className="empty">{access.session.status === "WAITING" ? "잠시만 기다려 주세요." : "첫 메시지를 보내보세요."}</div>}{messages.map((message) => <div key={message.id} className={`message ${message.senderType === "GUEST" ? "guest" : "agent"}`}><small>{message.senderType === "GUEST" ? "나" : "상담원"} · {new Date(message.createdAt).toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" })}</small><p>{message.content}</p></div>)}</section><footer><div className="timer"><span>남은 상담 시간</span><strong>{["CLOSED", "EXPIRED"].includes(access.session.status) ? "종료" : remainingTime(access.session.expiresAt, now)}</strong></div><form className="composer" onSubmit={send}><input aria-label="메시지" value={input} onChange={(e) => setInput(e.target.value)} maxLength={1000} placeholder={writable ? "메시지를 입력하세요" : "상담원 연결 또는 종료 상태를 확인하세요"} disabled={!writable}/><button disabled={!writable || !input.trim()}>전송</button></form></footer></main>;
}

ReactDOM.createRoot(document.getElementById("root")!).render(<React.StrictMode><GuestApp /></React.StrictMode>);
