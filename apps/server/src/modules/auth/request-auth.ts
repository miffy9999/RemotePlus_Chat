...
export async function requireStaff(request: Request, auth: AuthService, roles: Array<"ADMIN" | "AGENT">): Promise<StaffTokenPayload> {
  const identity = await auth.verifyActiveStaff(readBearerToken(request));
  if (!roles.includes(identity.role)) throw new UnauthorizedException("이 기능을 사용할 권한이 없습니다.");
  return identity;
}
...
export async function optionalStaff(request: Request, auth: AuthService): Promise<StaffTokenPayload | null> {
  if (!request.headers.authorization) return null;
  return auth.verifyActiveStaff(readBearerToken(request));
}
