...
describe("Agent 화면 레이아웃과 팝업 CSS 격리", () => {
  /** 현재 화면으로 되돌아가는 링크 하나뿐인 사이드바와 UI 언어 선택이 다시 나타나는 회귀를 막습니다. */
  it("기능 없는 사이드바와 UI 언어 선택을 렌더링하지 않는다", () => {
  ...
  it("활성 화면 알림 차단과 무음 시스템 알림 경계를 제공한다", () => {
    expect(mainSource).toContain('shouldNotifyAgent(document, modeRef.current === "log")');
