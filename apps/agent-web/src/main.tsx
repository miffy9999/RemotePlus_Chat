...
import { acceptSession, changeStaffPassword, closeSession, createAdminAgent, createHotel, createRoom, deleteAdminAgent, deleteHotel, deleteRoom, getMessages, listAdminAgents, listHotels, listRooms, listSessions, loginStaff, type AdminAgentView, type HotelView, type MessageView, type RoomView, type SessionView, SOCKET_URL } from "./api";
...
function showSystemNotification(title: string, body: string, tag: string): void {
  ...
  } catch { /* 브라우저 알림 실패와 관계없이 화면 팝업은 계속 표시됩니다. */ }
}

/** 하나의 최신 알림만 8초 동안 보여줘 연속 메시지가 상담 화면을 가득 덮지 않도록 합니다. */
function AgentNoticePopup({ notice, onClose, onAction }: { notice: AgentNotice; onClose: () => void; onAction: () => void }): React.JSX.Element {
  const { t } = useI18n();
...
function AgentPage({ auth }: { auth: AgentAuth }): React.JSX.Element {
  ...
  async function refresh(): Promise<void> {
    ...
    try {
      ...
      const assignedActive = data.filter((item) => item.status === "ACTIVE" && item.agentId === auth.agent.id);
      activeSessionsRef.current = new Map(assignedActive.map((item) => [item.id, item]));
      if (notificationSocketRef.current?.connected) {
        assignedActive.forEach((item) => notificationSocketRef.current?.emit("chat:join", { sessionId: item.id }));
      }
      ...
      if (newWaiting.length > 0) {
        ...
        const additional = newWaiting.length > 1 ? (language === "ja" ? ` · ほか${newWaiting.length - 1}件` : ` · 외 ${newWaiting.length - 1}건`) : "";
        announce({
      ...
      setError(reason instanceof Error ? reason.message : "상담 목록을 불러오지 못했습니다.");
    } finally {
  ...
  useEffect(() => {
    ...
    const joinAssignedSessions = () => activeSessionsRef.current.forEach((session) => socket.emit("chat:join", { sessionId: session.id }));
    ...
    return () => { socket.disconnect(); notificationSocketRef.current = null; };
  ...
  async function enableBrowserNotifications(): Promise<void> {
    ...
    if (permission === "granted") {
      setNotice({ id: `permission-${Date.now()}`, title: t("브라우저 알림이 켜졌습니다."), body: t("탭이 백그라운드여도 새 상담과 메시지를 알려드립니다.") });
    }
  }
...
function AdminPage({ auth }: { auth: AgentAuth }): React.JSX.Element {
  ...
  const [hotelError, setHotelError] = useState(""); const [roomError, setRoomError] = useState("");
  ...
  async function addHotel(e: FormEvent) { e.preventDefault(); setHotelError(""); try { await createHotel(auth.accessToken, hotelName); setHotelName(""); await refresh(); } catch (reason) { setHotelError(reason instanceof Error ? reason.message : "호텔 추가 실패"); } }
  /** 객실 중복 등 룸 생성 오류는 룸 입력 블록 바로 위에 표시하고 다른 관리자 오류와 섞지 않습니다. */
  async function addRoom(e: FormEvent) { e.preventDefault(); setRoomError(""); try { await createRoom(auth.accessToken, roomHotelId, roomNumber); setRoomNumber(""); await refresh(); } catch (reason) { setRoomError(reason instanceof Error ? reason.message : "룸 추가 실패"); } }
...
function Page({ auth, title, subtitle, chatMode = false, children }: React.PropsWithChildren<{ auth: AgentAuth; title: string; subtitle: string; chatMode?: boolean }>): React.JSX.Element {
  ...
  return <><div className={`shell ${chatMode ? "chat-shell" : ""}`}><main className={chatMode ? "chat-page" : ""}><header><div className="agent-brand">REMOTE<span>+</span></div><div className="page-heading"><h1>{title}</h1><p>{subtitle}</p></div><div className="agent-header-actions"><LanguageSwitcher/><div className="profile">{auth.agent.name}<span>AG</span><button className="link-button" onClick={() => setShowPasswordChange(true)}>{t("비밀번호 변경")}</button><button className="link-button" onClick={logout}>{t("로그아웃")}</button></div></div></header>{children}</main></div>{showPasswordChange && <PasswordChangeModal auth={auth} onClose={() => setShowPasswordChange(false)} />}</>;
}
