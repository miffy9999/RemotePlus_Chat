...
function SessionTable({ items, action }: { items: SessionView[]; action: (session: SessionView) => React.ReactNode }): React.JSX.Element {
  ...
  return <div className="table-wrap"><table><thead><tr><th>{t("호텔")}</th><th>{t("객실")}</th><th>{t("언어")}</th><th>{t("상태")}</th><th>{t("만료 시각")}</th><th>{t("작업")}</th></tr></thead><tbody>{items.length === 0 ? <tr><td colSpan={6} className="empty">{t("표시할 상담이 없습니다.")}</td></tr> : items.map((session) => <tr key={session.id}><td>{session.room.hotel.name}</td><td>{session.room.roomNumber}</td><td>{session.language.toUpperCase()}</td><td><span className={`badge ${session.status.toLowerCase()}`}>{session.status}</span></td><td>{new Date(session.expiresAt).toLocaleTimeString(locale)}</td><td>{action(session)}</td></tr>)}</tbody></table></div>;
}
...
function AdminLogin({ onLogin }: { onLogin: (auth: AgentAuth) => void }): React.JSX.Element { const {t}=useI18n(); const [loginId,setLoginId]=useState("admin"); const [password,setPassword]=useState(""); const [error,setError]=useState(""); async function submit(e:FormEvent){e.preventDefault();try{const value=await loginAdmin(loginId,password);sessionStorage.setItem("hotel-chat-admin-auth",JSON.stringify(value));onLogin(value);}catch(reason){setError(reason instanceof Error?reason.message:"로그인에 실패했습니다.");}} return <div className="login-shell"><LanguageSwitcher/><form className="login-card" onSubmit={submit}><div className="brand dark">REMOTE<span>+</span></div><h1>{t("관리자 로그인")}</h1><label>{t("로그인 ID")}<input value={loginId} onChange={e=>setLoginId(e.target.value)}/></label><label>{t("비밀번호")}<input type="password" value={password} onChange={e=>setPassword(e.target.value)}/></label>{error&&<div className="error-box">{error}</div>}<button>{t("로그인")}</button></form></div>; }
...
function Page({ auth, title, subtitle, children }: React.PropsWithChildren<{ auth: AgentAuth; title: string; subtitle: string }>): React.JSX.Element {
  ...
  return <div className="shell"><aside><div className="brand">REMOTE<span>+</span></div><LanguageSwitcher/><nav><Link to="/agent">{t("Agent 상담")}</Link><Link to="/admin">{t("관리자 페이지")}</Link></nav><small>Hotel Chat MVP</small></aside><main><header><div><h1>{title}</h1><p>{subtitle}</p></div><div className="profile">{auth.agent.name}<span>AG</span><button className="link-button" onClick={logout}>{t("로그아웃")}</button></div></header>{children}</main></div>;
}
