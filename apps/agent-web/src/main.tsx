...
function RoleSelection(): React.JSX.Element {
  return <div className="role-shell"><main className="role-card"><div className="brand dark">REMOTE<span>+</span></div><h1>호텔 상담 센터</h1><p>이용할 업무 화면을 선택한 뒤 로그인하세요.</p><div className="role-actions"><Link className="role-action admin" to="/admin/login"><strong>관리자 로그인</strong><span>호텔·룸·Agent를 관리합니다.</span></Link><Link className="role-action agent" to="/login"><strong>Agent 로그인</strong><span>투숙객 상담을 처리합니다.</span></Link></div></main></div>;
}
...
function AdminPage({ auth }: { auth: AgentAuth }): React.JSX.Element {
  ...
  async function removeAgent(agent: AdminAgentView) { if (!window.confirm(`${agent.name} (${agent.loginId}) Agent를 삭제할까요? 기존 상담 기록은 유지됩니다.`)) return; try { await deleteAdminAgent(auth.accessToken, agent.id); await refresh(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Agent 삭제 실패"); } }
  async function removeHotel(hotel: HotelView) { if (!window.confirm(`${hotel.name} 호텔을 삭제할까요? 하위 룸과 모든 상담·메시지도 함께 삭제되며 복구할 수 없습니다.`)) return; try { await deleteHotel(auth.accessToken, hotel.id); if (filter === hotel.id) setFilter(""); await refresh(); } catch (reason) { setError(reason instanceof Error ? reason.message : "호텔 삭제 실패"); } }
  async function removeRoom(room: RoomView) { if (!window.confirm(`${room.hotel.name} ${room.roomNumber} 룸을 삭제할까요? 접근키와 상담·메시지도 함께 삭제되며 복구할 수 없습니다.`)) return; try { await deleteRoom(auth.accessToken, room.id); await refresh(); } catch (reason) { setError(reason instanceof Error ? reason.message : "룸 삭제 실패"); } }
  function logout() { sessionStorage.removeItem("hotel-chat-admin-auth"); location.href = "/admin/login"; }
  return <div className="admin-shell"><header><div><div className="brand dark">REMOTE<span>+</span></div><h1>관리자 페이지</h1></div><button className="secondary" onClick={logout}>로그아웃</button></header>{error && <div className="error-box">{error}</div>}<section className="card"><h2>Agent 관리</h2><form className="inline-form" onSubmit={addAgent}><input aria-label="Agent 이름" placeholder="이름" value={agentForm.name} onChange={(e)=>setAgentForm({...agentForm,name:e.target.value})}/><input aria-label="Agent 로그인 ID" placeholder="로그인 ID" value={agentForm.loginId} onChange={(e)=>setAgentForm({...agentForm,loginId:e.target.value})}/><input aria-label="Agent 비밀번호" type="password" placeholder="영문+숫자 8자 이상" value={agentForm.password} onChange={(e)=>setAgentForm({...agentForm,password:e.target.value})}/><button>Agent 추가</button></form><table><thead><tr><th>이름</th><th>ID</th><th>상태</th><th>관리</th></tr></thead><tbody>{agents.map((agent)=><tr key={agent.id}><td>{agent.name}</td><td>{agent.loginId}</td><td>{agent.status}</td><td><button className="danger compact" onClick={() => void removeAgent(agent)}>삭제</button></td></tr>)}</tbody></table></section><section className="card"><h2>호텔·룸 관리</h2><form className="inline-form" onSubmit={addHotel}><input aria-label="호텔 이름" placeholder="호텔 이름" value={hotelName} onChange={(e)=>setHotelName(e.target.value)}/><button>호텔 추가</button></form><form className="inline-form" onSubmit={addRoom}><select aria-label="룸을 추가할 호텔" value={roomHotelId} onChange={(e)=>setRoomHotelId(e.target.value)}>{hotels.map((hotel)=><option key={hotel.id} value={hotel.id}>{hotel.name}</option>)}</select><input aria-label="객실 번호" placeholder="객실 번호" value={roomNumber} onChange={(e)=>setRoomNumber(e.target.value)}/><button>룸 추가</button><button type="button" className="danger" disabled={!roomHotelId} onClick={() => { const hotel = hotels.find((item) => item.id === roomHotelId); if (hotel) void removeHotel(hotel); }}>선택 호텔 삭제</button></form><label className="filter">호텔 필터<select value={filter} onChange={(e)=>setFilter(e.target.value)}><option value="">전체 호텔</option>{hotels.map((hotel)=><option key={hotel.id} value={hotel.id}>{hotel.name}</option>)}</select></label><div className="table-wrap"><table><thead><tr><th>호텔</th><th>객실</th><th>상태</th><th>투숙객 주소</th><th>QR 관리</th><th>관리</th></tr></thead><tbody>{rooms.map((room)=><tr key={room.id}><td>{room.hotel.name}</td><td>{room.roomNumber}</td><td>{room.status}</td><td className="room-link-cell">{room.guestUrl ? <><a href={room.guestUrl} target="_blank" rel="noreferrer">상담 링크 열기</a><button className="secondary compact" onClick={() => void copyGuestUrl(room.guestUrl!).catch(()=>setError("주소를 복사하지 못했습니다."))}>주소 복사</button></> : <span>주소 없음</span>}</td><td><button disabled title="QR 관리는 MVP 이후 제공됩니다.">준비 중</button></td><td><button className="danger compact" onClick={() => void removeRoom(room)}>삭제</button></td></tr>)}</tbody></table></div></section></div>;
}
