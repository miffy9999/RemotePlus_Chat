...
시드는 빈 DB에 기본 계정을 만들 때만 비밀번호를 사용하며 기존 직원 계정은 절대 갱신하지 않습니다. 따라서 Render가 코드 배포 과정에서 시드를 다시 실행하더라도 사용자가 변경한 비밀번호는 초기화되지 않습니다.

로그인 화면의 `아이디 저장`은 ID만 이 브라우저에 기억하고, `로그인 정보 저장`은 비밀번호를 앱 저장소에 평문으로 남기지 않고 브라우저 비밀번호 관리자에 저장을 요청합니다. 비밀번호 변경 화면에서는 각 입력칸 오른쪽 눈 아이콘으로 표시·숨김을 전환할 수 있습니다. 직원 인증 토큰은 두 선택과 관계없이 현재 탭의 `sessionStorage`만 사용합니다.

Render 무료 API는 15분 동안 HTTP 요청이나 WebSocket 메시지가 없으면 휴면하고 다음 요청에서 다시 시작되기까지 약 1분이 걸릴 수 있습니다. 로그인 페이지는 열릴 때 헬스 요청을 한 번 보내 기동을 앞당기고 긴 대기에는 안내를 표시하지만, 무료 플랜의 휴면을 완전히 없애지는 않습니다.
...
앱 내부 팝업은 별도 권한 없이 동작합니다. 다른 탭이나 창을 보고 있을 때도 운영체제 알림을 받으려면 HTTPS 운영 주소에서 대기 상담 영역의 `브라우저 알림 켜기`를 누르고 권한을 허용하세요. Edge를 포함한 Chromium 브라우저에서는 서비스 워커 알림을 우선 사용합니다. 같은 Agent ID를 여러 PC에서 공유하는 경우 브라우저 권한은 PC와 브라우저별로 각각 한 번 허용해야 합니다. 차단했다면 주소창의 사이트 정보에서 알림 권한을 허용한 뒤 새로고침하세요.

상담 제한 시간 15분은 고객이 QR에 접속한 때가 아니라 Agent가 `상담 수락`을 누른 시점부터 시작합니다. 수락 전에는 만료 시각이 없으며, 고객은 모바일 화면 상단의 `상담 종료`를 확인해 대기·진행 상담을 직접 끝낼 수 있습니다. 종료 상태는 Agent 목록과 열린 채팅에 Socket.IO로 즉시 반영됩니다. 단순 탭 닫기나 일시적인 네트워크 단절은 재접속 보호를 위해 종료로 취급하지 않습니다.

## 전체 상담 로그

Agent와 관리자는 화면 하단에서 담당자와 관계없이 종료·만료·취소·차단된 상담 기록을 호텔별로 필터링해 확인할 수 있습니다. `기록 보기`는 실시간 채팅방에 접속하지 않는 읽기 전용 모달이며 다른 Agent의 진행 중 상담은 공동 로그에 포함하지 않습니다.

대기·진행 상담은 새 요청 알림을 위해 5초마다 갱신하지만 완료 로그는 최초 진입, 상담 상태 변경, 60초 주기에만 별도로 조회합니다. 공개 상담 응답과 종료 이벤트에는 투숙객 토큰 해시나 직원 비밀번호 해시·토큰 버전을 포함하지 않습니다.
