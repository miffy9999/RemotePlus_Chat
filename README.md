...
통합 로그인에서 관리자 계정으로 로그인하면 호텔·룸 관리, 호텔별 자동 안내문, Agent 관리 순서의 대시보드가 표시됩니다. 호텔을 추가한 뒤 등록 호텔 칩 또는 선택창에서 대상 호텔을 골라 룸을 추가합니다. 서버가 추측하기 어려운 접근키를 자동 생성하며 룸 테이블에서 다음 기능을 제공합니다.
...
Agent 화면은 LINE형 `Current chat room / Log` 구조이며 상단 표시 언어에서 한국어·일본어를 전환할 수 있습니다. Guest는 Agent가 열기 전에도 메시지를 보낼 수 있고, Agent가 새 문의를 여는 순간부터 15분 상담 시간이 시작됩니다.
