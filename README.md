...
통합 로그인에서 관리자 계정으로 로그인하면 상단 운영 지표와 `호텔 관리 / Agent 관리 / 상담 로그` 메뉴가 있는 대시보드가 표시됩니다. 데스크톱에서는 왼쪽 메뉴를 접어 작업 공간을 넓힐 수 있고 모바일에서는 가로 탭으로 전환됩니다. `호텔 관리`는 호텔·룸과 Guest 자동 안내문을 하나의 카드로 묶어 제공합니다. 등록 호텔 칩 또는 룸 추가 선택창에서 호텔을 고르면 아래 자동 안내문도 같은 호텔로 즉시 바뀌므로 호텔을 두 번 선택할 필요가 없습니다. 룸을 추가하면 서버가 추측하기 어려운 접근키를 자동 생성하며 룸 테이블에서 다음 기능을 제공합니다.
...
Agent 화면은 LINE형 `Current chat room / Log` 구조이며 왼쪽 상단 계정 칩에서 현재 직원 이름을 확인하고, 표시 언어에서 한국어·일본어를 전환할 수 있습니다. 상담 입력창은 항상 화면 최하단에 있고 대화가 짧으면 최근 말풍선이 입력창 바로 위에 표시됩니다. Guest는 Agent가 열기 전에도 메시지를 보낼 수 있고, Agent가 새 문의를 여는 순간부터 정확히 15분 상담 시간이 시작됩니다. PC와 서버 시계가 달라도 타이머는 `15:00`을 넘겨 표시하지 않으며, 상담을 종료해도 `Log`로 자동 이동하지 않고 현재 대화가 읽기 전용으로 유지됩니다.
