...
Agent 화면은 LINE형 `Current chat room / Log` 구조이며 왼쪽 상단 계정 칩에서 현재 직원 이름과 로그인 ID를 확인하고, 표시 언어에서 한국어·일본어를 전환할 수 있습니다. 상담 입력창은 항상 화면 최하단에 있고 대화가 짧으면 최근 말풍선이 입력창 바로 위에 표시됩니다. Guest는 Agent가 열기 전에도 메시지를 보낼 수 있고, Agent가 새 문의를 여는 순간부터 15분 상담 시간이 시작됩니다.
