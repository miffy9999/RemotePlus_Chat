...
`Log` 탭은 완료된 기록 전체를 한 번에 내려받지 않고 서버와 PostgreSQL에서 최근 활동 순으로 100건씩 조회합니다. 이전·다음 버튼으로 페이지를 이동하며 검색어·호텔·언어 조건도 서버 쿼리에 포함되므로, 기록이 많아져도 브라우저가 전체 로그를 보관하거나 서버가 전체 행을 전송하지 않습니다. 5초 자동 갱신은 대기·진행 상담만 조회하고 종료 로그 페이지는 별도 요청으로 분리합니다.
