...
- Agent 카운트다운: `apps/agent-web/src/main.tsx`, `chat-utils.ts`
- 타이머 회귀 테스트: `apps/agent-web/src/chat-utils.test.ts`
- 문서: `docs/11_User_Manual.md`

### 수정 내용과 이유

- PC 시계가 서버보다 느리면 서버 만료 시각과 로컬 시각의 차이가 15분을 넘고, 기존 계산이 매초 결과를 `15:00`으로 잘라 시계 오차가 해소될 때까지 타이머가 멈춰 보였다.
- 서버 `expiresAt`을 처음 받은 렌더링에서 최대 15분인 로컬 카운트다운 종료 시각을 한 번만 고정한다. 이후에는 그 고정 시각을 기준으로 매초 계산해 `15:00 → 14:59 → 14:58`로 감소한다.
- 서버의 실제 만료 시각, 15분 정책, 종료 이벤트와 메시지 차단 로직은 변경하지 않았다.

### 확인 방법

- 서버 만료 시각이 로컬 기준 17분 뒤로 계산되는 시계 오차 사례에서도 첫 표시 `15:00`, 1초 뒤 `14:59`가 되는 단위 테스트를 추가했다.
- Agent typecheck, 84개 테스트와 프로덕션 build를 통과했다.
- Docker 실제 화면에서 첫 Agent 답변 직후 `15:00`, 약 2초 뒤 `14:58`로 감소하는 것을 확인했다.
- 검증용 상담은 정상 종료해 정리했다.
...
- 호텔 선택 연동 UI: `apps/agent-web/src/main.tsx`, `i18n.tsx`, `styles.css`, `dashboard-layout.test.ts`
- Agent 언어 필터: `apps/agent-web/src/session-filters.ts`, `session-filters.test.ts`, `main.tsx`
- 문서: 원본 사양서, `README.md`, `docs/07_UI_Structure.md`, `08_System_Blueprint.md`, `09_Analysis_Roadmap.md`, `11_User_Manual.md`, `12_Feature_Status.md`

### 수정 내용

- 룸 추가용 호텔 선택과 자동 안내문 대상 호텔을 `roomHotelId` 하나로 통합하고 안내문 영역의 중복 호텔 선택창을 제거했다.
- 호텔 칩 또는 룸 추가 선택창에서 호텔을 바꾸면 아래 현재 선택 호텔 표시와 일본어·영어 안내문 원문이 즉시 같은 호텔로 전환된다.
- Agent 언어 필터 옵션에 실제 소문자 언어 코드 값을 명시하고, 상담 데이터도 소문자로 정규화해 `JA` 표시와 `ja` 데이터가 정확히 일치하게 했다.
- 검색·호텔·언어 조건을 순수 필터 함수로 분리해 조합 동작을 단위 테스트할 수 있게 했다.

### 수정 이유

같은 호텔 관리 카드에서 호텔을 두 번 선택하는 중복 UI를 없애고, 대문자 옵션값과 소문자 API 언어 코드 불일치로 언어 필터가 정상 적용되지 않던 문제를 해결하기 위해서다.

### 확인 방법

- Agent 웹 단위 테스트 65개가 통과했다.
- 브라우저에서 `JA`, `EN`, `KO`를 각각 선택해 해당 언어 상담만 한 건씩 남는지 확인했다.
- 관리자 호텔 선택을 `타누키`에서 `사쿠라 하우스`로 바꿨을 때 아래 호텔 표시와 안내문 원문이 즉시 함께 바뀌고 안내문 영역의 호텔 select가 0개인지 확인했다.

## 2026-07-23 16:23:00 +09:00

### 수정한 파일

- Agent 종료·타이머: `apps/agent-web/src/main.tsx`, `chat-utils.ts`, `chat-utils.test.ts`, `conversation-logs.test.ts`
- 관리자 통합 카드: `apps/agent-web/src/main.tsx`, `styles.css`, `dashboard-layout.test.ts`
- 문서: 원본 사양서, `README.md`, `docs/07_UI_Structure.md`, `08_System_Blueprint.md`, `09_Analysis_Roadmap.md`, `11_User_Manual.md`, `12_Feature_Status.md`

### 수정 내용

- Agent가 상담을 종료해도 `Log` 탭으로 자동 전환하지 않고 현재 대화를 읽기 전용 상태로 유지한다.
- 서버의 실제 15분 정책은 유지하면서 PC와 서버 시계 차이로 `17:00`처럼 계산된 화면 타이머를 최대 `15:00`으로 제한한다.
- 호텔·룸 관리와 호텔별 Guest 자동 안내문을 두 개의 카드에서 하나의 호텔 관리 카드 내부 하위 섹션으로 통합했다.

### 수정 이유

상담 종료 직후 상담원의 화면 맥락을 보존하고, 시계 오차가 운영 제한시간을 잘못 안내하지 않게 하며, 같은 호텔 설정이 불필요하게 분리돼 보이는 관리자 UI를 정리하기 위해서다.

### 확인 방법

- Agent 웹 단위 테스트 61개가 통과했다.
- 만료시각을 클라이언트 기준 17분 뒤로 준 브라우저 QA에서도 타이머가 `15:00`으로 표시되는지 확인했다.
- 종료 API 응답 뒤 `Current chat room`이 활성 상태이고 현재 본문·종료 안내가 남는지 확인했다.
- 관리자 호텔 화면의 최상위 카드가 1개이고 그 안에 호텔·룸과 자동 안내문 제목이 모두 있는지 확인했다.
...
- Agent 레이아웃: `apps/agent-web/src/main.tsx`, `styles.css`, `i18n.tsx`, `agent-layout.ts`
- Agent 테스트: `apps/agent-web/src/agent-layout.test.ts`
- Guest 레이아웃: `apps/guest-web/src/styles.css`
- 문서: `docs/07_UI_Structure.md`, `docs/11_User_Manual.md`, `docs/12_Feature_Status.md`

### 수정 내용

- Agent 화면 높이를 동적 뷰포트에 맞추고 사이드바 헤더가 줄바꿈되도록 하여 화면 확대·축소 또는 낮은 화면에서 목록 하단이 잘리지 않게 했다.
- Agent 목록과 대화창 사이에 마우스·터치 드래그 및 방향키로 너비를 조절할 수 있는 구분선을 추가했다.
- 목록 너비는 320~620px 범위와 현재 화면의 대화창 최소 폭 안에서 제한하고, 사용자가 정한 값을 브라우저에 저장한다.
- 900px 이하에서는 기존 모바일 단일 화면 전환을 유지하며 리사이저를 숨긴다.
- Agent와 Guest 메시지 입력 영역을 최하단 고정 행으로 유지하고, 메시지 영역만 스크롤되도록 뷰포트·overflow·safe-area 처리를 보강했다.

### 수정 이유

큰 화면과 확대된 화면에서 Agent 사이드 목록 일부가 잘리는 문제를 해결하고, 상담원이 호텔·객실 목록과 대화 본문 비율을 업무 방식에 맞게 직접 조정할 수 있게 하기 위해서다.

### 확인 방법

- 사이드바 너비 최소·최대·화면별 제한, 저장·복원 단위 테스트를 추가했다.
- `pnpm lint`와 전체 운영 빌드가 성공했다.
- `pnpm test`에서 서버 53개, Agent 54개, Guest 14개 테스트가 전부 성공했다.
- Docker의 Agent·Guest 웹을 최신 소스로 재빌드하고 실제 브라우저에서 목록 너비 420px → 436px 키보드 조절과 새로고침 후 복원, Agent 입력창 하단 좌표와 메시지 내부 스크롤을 확인했다.
- Guest는 조건부 안내문이 없을 때 빈 그리드 행이 입력창 아래 공간을 차지하는 원인을 추가 발견해 세로 flex 구조로 교체했고, 입력창 하단과 뷰포트 하단이 같은 좌표인지 확인했다.
