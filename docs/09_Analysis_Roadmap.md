...
- 관리자 대시보드는 호텔·Agent·상담 로그 작업을 어떻게 한 화면에서 전환하는가?
...
3. 상담 목록 상태 관리
4. 채팅 메시지 상태 관리
5. `LineConversationPanel`의 헤더·본문·입력창 3행 레이아웃
6. `chat-utils.ts`의 15분 표시 상한과 종료 이벤트 뒤 현재 탭 유지 처리
7. `session-filters.ts`의 검색·호텔·대소문자 안전 언어 필터
8. `AdminPage`의 단일 호텔 선택 상태, 호텔 통합 카드와 Agent·로그 조건부 패널
9. 반응형 화면 컴포넌트와 `styles.css`
