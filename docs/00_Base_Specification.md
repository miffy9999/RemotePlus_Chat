...
향후 호텔 객실에 비치할 QR 코드 또는 전용 링크를 통해 투숙객이 별도의 앱 설치나 회원가입 없이 콜센터 직원과 실시간으로 채팅할 수 있도록 한다. 단, 초기 MVP에서는 QR 코드 생성·배포를 구현하지 않고 테스트용 전용 링크 또는 접근 키로 투숙객 채팅 흐름을 검증한다.

채팅 세션은 객실을 기준으로 생성하며, 한 번의 상담은 최대 15분 동안만 사용할 수 있다. 상담원이 종료하거나 제한 시간이 지나면 해당 세션에서는 더 이상 메시지를 보낼 수 없어야 한다.

### MVP 목표

> 테스트용 전용 링크 또는 접근 키로 접속한 투숙객과 로그인한 콜센터 직원이 최대 15분 동안 안전하게 실시간 텍스트 상담을 진행할 수 있다. QR 코드는 후속 단계에서 같은 접근 키 흐름에 연결한다.
...
룸 테이블 마지막 열에는 향후 QR 관리 버튼이 들어갈 자리를 고려한다. QR 생성, 미리보기, 다운로드, 삭제 및 재생성 기능은 화면·데이터 구조와 API 확장 지점만 문서화하고 MVP에서는 구현하지 않는다.

---

## 5. MVP 제외 범위

- QR 코드 생성, 미리보기 및 다운로드
- QR 코드 삭제, 폐기 및 재생성
- 이미지 및 파일 전송
- 음성·영상 통화
- 자동 번역
...
향후 룸 테이블의 QR 버튼을 누르면 해당 룸의 QR 코드를 확인하고 이미지로 다운로드할 수 있다. 기존 QR 코드가 노출되거나 사용할 수 없는 경우 관리자가 삭제 또는 폐기한 뒤 새 QR 코드를 만들 수 있어야 한다. 재생성 시 기존 QR과 연결된 접근 키는 즉시 비활성화하고 새 접근 키를 발급한다. 이 QR 수명주기 기능은 MVP 이후 범위다.

### 투숙객

```mermaid
flowchart TD
    A[테스트용 링크 접속] --> B[접근 키 검증]
    B -->|성공| C[상담 시작]
    B -->|실패| X[접근 오류 표시]
    C --> D[상담 세션 생성]
    D --> E[상담원 연결 대기]
