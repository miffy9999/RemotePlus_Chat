...
채팅 세션은 객실을 기준으로 생성한다. Guest는 Agent 배정 전 `WAITING`과 담당자가 배정됐지만 첫 답변 전인 `ACTIVE`에서도 메시지를 보내 DB에 저장할 수 있으며 대기 시간만으로 만료되지 않는다. 15분 제한은 담당 Agent의 첫 답변 메시지가 DB에 저장된 시점부터 계산하고, 상담원이 종료하거나 제한 시간이 지나면 더 이상 메시지를 보낼 수 없어야 한다.
...
- 대화를 여는 동작은 담당 Agent만 배정한다. 서버 정책은 담당 Agent의 첫 답변 메시지가 DB에 저장된 시점부터 정확히 15분이며, Agent 화면 타이머는 PC와 서버 시계 차이가 있어도 `15:00`을 초과해 표시하지 않는다.
- Agent 언어 필터는 옵션의 실제 값으로 소문자 언어 코드(`ja`, `en`, `ko`, `zh`)를 사용하고 상담 데이터와 대소문자에 관계없이 비교한다.
- 객실 식별 정보
- 상담 시작 시각
- 남은 시간
- 사용 언어
...
- 왼쪽 관리 메뉴와 오른쪽 작업 패널로 이루어진 일반적인 운영 대시보드 레이아웃. 등록 Agent·호텔·룸·Guest 주소 수를 반복 표시하는 상단 요약 지표는 두지 않음
- Agent 상담 목록 폴링은 일시적인 Render·네트워크 실패 한두 번을 사용자 오류로 노출하지 않고 기존 목록을 유지해 자동 재시도함. 3회 연속 실패한 경우에만 재연결 안내를 표시하고 성공 즉시 해제함
- ADMIN 계정의 상단 사용자 이름은 시드·DB 이름이나 현재 UI 언어와 무관하게 일본어 `管理者`로 고정 표시하고, 일반 Agent는 등록된 직원 이름을 표시함
...
- 호텔 칩 또는 룸 추가 영역의 호텔 선택값을 카드 전체의 단일 선택 상태로 사용하며, 아래 자동 안내문에서 호텔을 다시 선택하지 않는다. 상단 호텔을 바꾸면 안내문 원문도 즉시 같은 호텔로 전환한다.
