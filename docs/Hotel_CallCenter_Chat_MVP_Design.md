...
- Agent 업무는 상담 목록과 채팅으로 구성된 단일 흐름이므로 현재 화면으로 다시 이동하는 전용 사이드바를 두지 않는다.
- 로고, 언어 선택, Agent 정보와 로그아웃은 상단 헤더에 모아 상담 영역의 가로 공간을 확보한다.
- Agent 헤더에는 현재 로그인한 직원 이름을 보여 공용 콜센터 PC에서도 사용 계정을 즉시 식별하게 한다.
- 선택한 상담 본문은 `상단 정보 / 스크롤 가능한 메시지 / 하단 입력창`의 세 행으로 고정한다. 읽기 전용 안내나 오류가 조건부로 나타나도 입력창은 항상 브라우저 화면 최하단 행을 유지한다.
- 메시지가 적을 때는 최근 말풍선이 하단 입력창 바로 위에서 시작하고, 메시지가 많을 때는 메시지 영역만 내부 스크롤한다.
- Agent가 상담을 종료하면 현재 대화 본문을 종료된 읽기 전용 상태로 그대로 유지하고 `Log` 탭으로 자동 이동하지 않는다.
- 서버 정책은 대화 열기 시점부터 정확히 15분이며, Agent 화면 타이머는 PC와 서버 시계 차이가 있어도 `15:00`을 초과해 표시하지 않는다.
- 객실 식별 정보
- 상담 시작 시각
- 남은 시간
- 사용 언어
...
- 관리자와 Agent는 같은 직원 로그인 화면을 사용하고, 인증된 계정 역할에 따라 관리자 페이지 또는 Agent 페이지로 자동 이동
- 로그인 이후 관리자 페이지와 Agent 페이지의 URL·API 권한은 역할로 구분
...
- 상담 시작 시간 및 남은 시간 확인
- 상담 종료 버튼
- 모든 Agent의 완료 상담 로그 공동 조회와 호텔별 필터
- 관리자의 동일 사양 완료 상담 로그 조회
- 누적 로그 목록의 제한 높이와 내부 스크롤
- 새로운 상담 또는 메시지의 화면상 표시
- 새 상담과 고객 메시지를 8초 동안 표시하는 화면 우측 상단 팝업
- 새 상담과 고객 메시지는 기본 무음 화면 알림으로 제공하고 Agent가 알림음을 직접 켜거나 끌 수 있음
...
- 상단 운영 지표와 왼쪽 관리 메뉴, 오른쪽 작업 패널로 이루어진 일반적인 운영 대시보드 레이아웃
- 관리 메뉴에서 `호텔 관리`, `Agent 관리`, `상담 로그`를 구분하고 한 번에 선택한 작업 패널 하나만 표시
- 데스크톱에서는 관리 메뉴를 접어 아이콘 중심의 좁은 탐색 영역으로 전환할 수 있고, 모바일에서는 같은 메뉴를 가로 탭으로 제공
- `호텔 관리` 작업 패널은 호텔·룸 CRUD와 호텔별 Guest 자동 안내문을 하나의 카드 안에서 연속된 하위 섹션으로 제공
...
- 이메일·SMS·운영체제 푸시 알림
- 다중 호텔 관리 화면
- 조직별 세부 권한·승인 워크플로를 포함한 엔터프라이즈 관리자 기능
- 객실 서비스 주문
- 결제
- 장기간 상담 내역 보존
