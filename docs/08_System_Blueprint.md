...
| `notifications` | 새 상담과 새 메시지의 화면 팝업, 선택형 알림음 및 Edge 서비스 워커 브라우저 알림 |
| `operations` | 로그, 헬스 체크, 감사 기록 |
| `jobs` | ACTIVE 만료 대상 검색과 Agent 수락 뒤 15분 자동 종료 |
...
stateDiagram-v2
    ...
    ACTIVE --> CLOSED: Agent 또는 고객이 정상 종료
    ACTIVE --> EXPIRED: 15분 만료
    CLOSED --> [*]
    EXPIRED --> [*]
```

WAITING은 `expiresAt = null`이며 Agent가 수락할 때 `startedAt`과 15분 뒤 `expiresAt`을 함께 기록한다. 서버는 메시지를 저장하기 직전에 세션이 `ACTIVE`인지, 현재 시각이 `expiresAt` 이전인지 다시 확인한다. 화면의 타이머는 안내용이며 최종 판단 기준이 아니다.
...
- 저장값은 WAITING이거나 브라우저 만료 시각 이전의 ACTIVE이면서 서버 `GET /chat-sessions/:id` 검증을 통과할 때만 사용한다. 손상·만료·종료·인증 거부된 해당 접근 키 항목은 제거하고 신규 상담 동의 화면으로 복구한다.
- React 개발 모드의 중복 실행과 동시 요청에도 열린 상담이 두 개 생기지 않도록 클라이언트 실행 가드와 데이터베이스 부분 고유 인덱스를 함께 사용한다.
- Agent·관리자는 `POST /auth/login` 한 번으로 계정 역할을 판별하고 해당 페이지로 자동 이동한다. 인증은 역할별 키를 사용하는 탭 단위 `sessionStorage`에 두고 탭 종료 또는 로그아웃 때 제거한다. 선택한 저장 모드와 ID만 `localStorage`에 기록하고 비밀번호 저장은 브라우저 Credential Management/비밀번호 관리자에 위임한다. 24시간 콜센터 운영을 위해 직원 JWT 자체에는 만료 시각을 넣지 않되, 모든 REST·WebSocket 인증에서 현재 계정의 존재·활성 상태·역할을 DB로 다시 확인한다.
...
- 시드는 새 DB에만 기본 테스트 비밀번호를 만들고 기존 직원 계정의 update 경로는 항상 빈 객체로 유지한다. 과거 재설정 환경변수가 Render에 남거나 시드가 다시 실행돼도 비밀번호·토큰 버전을 덮어쓸 수 없어야 한다.
- Render 무료 Web Service는 유료 전용 pre-deploy 명령을 사용할 수 없으므로 DB 마이그레이션을 시작 명령에서 유지한다. 빌드 단계에서 운영 DB를 변경하는 방식은 배포 실패 시 스키마와 실행 코드가 어긋날 수 있어 사용하지 않는다.
...
- 목록 API는 `OPEN`과 `COMPLETED` 범위를 분리한다. 5초 폴링은 `OPEN`만 조회하고 공동 완료 로그는 최초 진입·상태 변경·60초 주기에 조회해 30일치 로그가 매 폴링 응답에 반복 포함되지 않게 한다.
- Agent·관리자의 완료 기록 상세는 REST 이력만 불러오는 공용 읽기 전용 모달이며 Socket.IO 방 입장이나 메시지 전송 UI를 만들지 않는다.
...
- 시스템 알림은 브라우저 권한이 `granted`이고 Agent 탭이 백그라운드일 때만 표시한다. Edge 호환을 위해 서비스 워커 `showNotification()`을 우선 사용하고 직접 Notification 생성자는 폴백으로만 사용하며, 권한 거부나 등록 실패 환경에서도 화면 팝업과 제목 깜빡임은 유지한다.
...
- 공개 상담 응답과 종료 WebSocket 이벤트는 공용 직렬화 경계를 거쳐 `guestTokenHash`, Agent `passwordHash`, `tokenVersion` 등 인증 내부값을 제거하고 Agent의 `id`, `name`만 제공한다.
- 서버는 시작 직후와 5초마다 만료 시각이 지난 ACTIVE만 DB에서 조회해 `EXPIRED`로 원자 전환하고 Socket.IO 종료 이벤트를 보낸다. 고객의 명시적 종료도 토큰 검증 뒤 같은 종료 이벤트를 고객 방과 직원 공용 방에 보내 Agent 목록을 즉시 갱신한다.
- 종료된 상담 기록은 `closedAt` 기준 30일 보존하며 서버 시작 후 1회와 이후 24시간마다 한 번의 인덱스 삭제 쿼리로 정리한다. 세션 삭제 시 메시지는 DB 연쇄 삭제한다.
- REST는 기본적으로 IP·메서드·경로별 분당 제한을 적용한다. 직원 상담 목록 GET은 같은 NAT의 여러 PC를 위해 원본을 저장하지 않는 `IP + Bearer 토큰 지문` 단위로 제한하고, WebSocket 메시지는 연결별 분당 60개로 제한한다.
