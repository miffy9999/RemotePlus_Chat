...
- 공통 직원 로그인에서 ADMIN 역할을 `/admin/agent`로 자동 이동하고 Dashboard는 명시적 버튼으로 진입
- Agent·호텔·룸·Guest 주소 상단 운영 지표
- 접을 수 있는 데스크톱 왼쪽 메뉴와 모바일 가로 탭에서 `호텔 관리 / Agent 관리` 작업 패널 전환
- 하나의 호텔 관리 카드 안에 이어지는 호텔·룸 관리, 등록 호텔 빠른 선택과 호텔별 자동 안내문
- 호텔 칩·룸 추가 호텔 선택을 자동 안내문과 공유해 중복 선택 없이 같은 호텔 원문으로 전환
- 호텔별 룸 추가 및 테이블 조회
- `selectedHotelId=null`인 전체 호텔 버튼과 실제 호텔 ID 버튼을 이용한 룸 API 필터
- 일본어·영어 호텔별 Guest 자동 안내문
- Agent 목록 및 추가
...
- 공통 직원 로그인에서 AGENT는 `/agent`, ADMIN은 `/admin/agent`로 자동 이동
- 반복 이동 기능이 없는 사이드바를 제외하고 로고·언어·계정 동작을 상단 헤더에 통합
- 공용 PC에서 현재 직원 이름을 보여주는 계정 칩
- LINE형 `Current chat room`의 전체 WAITING·본인 ACTIVE와 `Log`의 전 Agent 완료 기록
- 공동 완료 로그의 서버 검색·호텔·언어 필터와 100건 단위 이전·다음 페이지
- 별도 수락 화면 없는 대화 열기와 한국어·일본어 UI
- 소문자 언어 코드를 명시적으로 사용하는 검색·호텔·언어 필터
- 조건부 안내·오류와 무관하게 유지되는 헤더·메시지·하단 입력창 3행 구조와 짧은 대화 하단 정렬
- 종료 후 현재 대화의 읽기 전용 전환과 수동 `Log` 탭 선택
- PC·서버 시계 오차와 무관하게 15분을 넘지 않는 Agent 타이머 표시
- 실시간 메시지 송수신
- 객실, 언어, 상담 상태, 남은 시간 확인
- API의 `agentId`와 공개 `agent { id, name }` 관계를 이용한 실제 담당 상담원 표시와 실시간 갱신
- 상담 수동 종료

### 투숙객 페이지

- 테스트 링크 또는 접근 키 검증
- 호텔 SYSTEM 안내문과 WAITING 상담 생성
- 상담원 연결 전 메시지 즉시 저장
- 실시간 메시지 송수신
- 연결 상태와 남은 시간 확인
- 종료 후 입력 차단
...
| `notifications` | 비활성 Agent 화면의 새 상담·새 메시지 무음 팝업 및 브라우저 알림 |
...
flowchart TD
    ...
    Create --> Waiting["WAITING + expiresAt null\n호텔 SYSTEM 안내문 + guestToken"]
    Login["POST /auth/login"] --> Role{"응답 역할"}
    Role -->|AGENT| StaffToken["Agent JWT"]
    Role -->|ADMIN| AdminAgent["ADMIN Agent 전체 조회"]
    AdminAgent -->|관리자 페이지 버튼| AdminPage["관리자 Dashboard"]
...
- 시스템 알림은 브라우저 권한이 `granted`이고 Agent 탭이 백그라운드일 때만 `silent: true`로 표시한다.
- ADMIN이 선택한 WAITING/ACTIVE 상담은 메시지 전송 권한을 계속 차단하되 상담별 Socket.IO 방에 읽기 전용으로 입장한다. 따라서 Agent 또는 Guest가 저장한 `chat:message`가 관리자 대화창에도 즉시 병합된다.
...
- 완료 로그 페이지는 `ChatSession(status, lastActivityAt)` 복합 인덱스로 상태 범위를 먼저 좁힌 뒤 최신 활동 순으로 읽어 전체 스캔·전체 응답을 피한다.
