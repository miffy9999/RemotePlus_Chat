...
5. 입력을 요구하는 환경변수에는 다음 값을 설정합니다.
   ...
   - `SEED_RESET_EXISTING_PASSWORDS`: 비밀번호 변경값을 재배포 후에도 유지하도록 `false`
...
기존 무료 DB에서 과거 테스트 비밀번호 마이그레이션이 이미 처리된 경우 최종 `20260722063500_enforce_free_test_credentials` 마이그레이션이 현재 문서의 두 계정을 한 번만 강제 복구합니다. 이후 서버 시작은 비밀번호를 덮어쓰지 않습니다. 새 Vercel이 `scope` 목록 쿼리를 먼저 사용해도 구버전 Render가 배포 중이면 일시적으로 전체 목록을 받아 필요한 상태만 필터링합니다.
