...
6. 배포 후 `https://<agent-project>.vercel.app/login`에서 ADMIN·AGENT 계정의 역할별 자동 이동을 확인합니다. 기존 `/admin/login` 주소는 공통 로그인으로 이동합니다.
7. 비밀번호를 변경했다면 Render 재배포 후에도 새 비밀번호로 로그인되고 이전 비밀번호가 거부되는지 확인합니다.

과거 세 비밀번호 복구 마이그레이션은 당시 DB를 한 번 복구하기 위한 이력이며 새 복구 마이그레이션을 추가하지 않습니다. 현재 시드는 신규 직원 계정만 생성하고 기존 계정의 update는 항상 빈 객체이므로 Render가 시드를 다시 실행해도 변경한 비밀번호를 덮어쓰지 않습니다. Render Dashboard의 Environment에 과거 `SEED_RESET_EXISTING_PASSWORDS`, `FORCE_FREE_TEST_CREDENTIALS`가 남아 있다면 혼동 방지를 위해 삭제할 수 있으며 현재 코드는 두 값을 사용하지 않습니다. 새 Vercel이 `scope` 목록 쿼리를 먼저 사용해도 구버전 Render가 배포 중이면 일시적으로 전체 목록을 받아 필요한 상태만 필터링합니다.
...
7. Agent·관리자 공동 로그가 읽기 전용 모달로 열리고 브라우저 네트워크 응답에 `guestTokenHash`, `passwordHash`, `tokenVersion`이 없는지 확인합니다.

## 무료 플랜 제한

- Render 무료 Web Service는 15분간 HTTP 요청이나 WebSocket 메시지가 없으면 중지되며, 첫 재접속에는 최대 약 1분의 준비 시간이 생길 수 있습니다.
- 직원 로그인 페이지는 열릴 때 `/api/health`를 한 번 요청해 사용자가 ID를 입력하는 동안 기동을 앞당기고, 로그인이 4초를 넘으면 서버 기동 안내를 표시합니다. 무료 인스턴스의 휴면 자체를 없애지는 않습니다.
- 무료 인스턴스를 계속 깨우는 외부 핑은 무료 시간·대역폭을 소모하므로 사용하지 않습니다. 콜드 스타트를 완전히 제거하려면 유료 Web Service 또는 상시 실행 VPS로 이전해야 합니다.
...
- 코드 배포 뒤 비밀번호가 바뀌면 새 배포 로그에서 `prisma:seed` 실행 여부를 확인하되, 현재 버전의 시드는 기존 직원 해시를 갱신할 수 없습니다. Render가 아직 구버전이면 최신 `main` 커밋으로 Manual Deploy를 한 번 실행합니다.
